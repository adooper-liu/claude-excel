...
**ERP 定位（已定）：** 店小秘 / 马帮 / 积加 / 领星等是**上游数据伙伴（水厂）**，不是竞品。平台 raw 数据经 ERP 鉴权与清洗后，Pack 内 **connector（L3）** 拉取同构表进 Excel；核心底座**不出现**任何 ERP 品牌与字段映射。SheetWise 做入户管道 + 分户计量 + 阀门，做 ERP 不做的本机假设/对账/透视最后一公里。
...
1. **有消费者才建** — 每份文档必须有明确的「谁读、何时读、怎么用」。没有消费者的文档不建。
2. **单一真相，不复制** — 同一事实只写在一处。其他文档引用它，不重复。链接用相对路径。
3. **分层管理** —
   - **运行时文档**（SKILL.md / knowledge / manifest / schema / fixtures）：必须与代码同步，错误直接影响行为。改代码同步改文档。
   - **参考文档**（docs/）：可「尽力准确」。状态标注（已定/已落地/随时更新），过时标「已过时」或删除。
4. **死文档要清** — 不再维护或超过 6 个月未更新的参考文档，标「已过时」或删除。任务 brief 合 master 后保留但不再修改。
5. **新建过门** — 新建文档前问三句：能并进现有文档吗？消费者是谁？谁来维护？

**文档消费链**：见 `docs/document-usage.md`（每份文档的「谁读、何时读、怎么用」清单）。
