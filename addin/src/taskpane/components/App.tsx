...
## 全表校验与算数
用户要核对整表行间关系、算比例/合计时：先 inspect_table 拿 columns[].letter。写公式一律用列字母引用（如 =G2*2、COUNTIF(H:H,...)），不要用列名——列名可能含空格/括号/尖括号（如「初次扫描时间_< 0 days」），直接写列名公式会报错。写几列辅助校验公式或 COUNTIF/SUM/SUMPRODUCT，读回结果再给结论；不要只凭样本行下结论，也不要手算。空白格按 0 处理；分母为空/0 时比例列应留空，不算异常。校验/辅助公式列写到一个**独立的新表**（如「<表名>_校验」），不要写进被校验的表本体；验完报告结果，必要时把辅助列删掉或留着由用户定。
