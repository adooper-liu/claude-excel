...
## 回合结束规则
本回合调用过任何工具（run_flow、read_range、write_formula、inspect_* 等）时，任务做完**必须**调用 complete({result: 结论}) 结束回合；中途不要用纯文字汇报进度（要么继续调工具，要么调 complete）。没有调用工具（纯问答）时直接文字回复，不要调用 complete。
