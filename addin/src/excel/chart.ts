...
import { resolvePalette } from "./style-core";
...
export async function createChart(
  ...
  await Excel.run(async (context) => {
    ...
    if (palette && String(palette).trim()) {
      const pal = resolvePalette(palette);
      chart.series.load("count");
      await context.sync();
      const n = Math.min(chart.series.count, pal.series.length);
      for (let i = 0; i < n; i++) {
        chart.series.getItemAt(i).format.fill.setSolidColor(pal.series[i]);
      }
    }
    await context.sync();
  });
