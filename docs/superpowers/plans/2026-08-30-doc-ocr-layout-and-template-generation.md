...
- 2026-08-31 真机发现并修复：密集发票表单下 rapid 路径质量倒退（原实现 0 kvs + 乱表）。根因：PP-Layout CDLA 把整页表单分类为 table，“文本区抽键值”全丢；RapidOCR 为行级盒，贰给 tesseract 词盒聚类逻辑全崩。修复：raw_text/kvs 改用 RapidOCR 全文（按位置排序，不重聚类），整页表单区（面积>50%）跳过 RapidTable，改用位置重建表格（行按 y1+全局中位盒高分行、列数一致性切分）；first_table 优先选带表头的表。实测：该发票 14 个抬头键值 + 明细表（8列表头+1 行），first_table 正确选中明细表；tesseract 词盒聚类同图为 0 kvs + 0 tables。门槦：后端 334 passed + 2 skip，venv 真实 rapid 16 passed。
- 2026-08-31 数据驱动地局式装与区块机制（应对变化单据，非针对单一模板）：OCR 对象多变，不能靠页面模型分区（密集表单会把整页分为 table）。机制：（1）raw_text/kvs 用 RapidOCR 全文正则（通用）；（2）噪声块识别：符号密集、无汉字/字母的盒集合为区块（如发票密码区 4 个字符串聚成一块），整块从表格/键值中排除；（3）位置重建表格：行按 y1+全局中位盒高分行，列数一致性切分，first_table 优先带表头的表；（4）局部表格区仍走 RapidTable。实测同图：14 kvs + 明细表（8列表头+1 行）+ 合计表，first_table=明细表；密码区单独噪声块被识别并排除。门槦：后端 337 passed + 2 skip，venv 真实 rapid 19 passed。
