...
- [ ] **⑧ 模板打标记定位（position 锚点增强）**（方案见 `docs/ocr-layout-design.md`
      「模板打标记定位：最佳实践」）
  - [ ] 模板字段加可选 `position`：创建模板时自动记录归一化锚点（0..1 相对坐标，
        detail 可相对表格 bbox），不存绝对像素。
  - [ ] 解析时对照本次 OCR 盒图就近匹配（容差 ±10~15%）：语义匹配失败/歧义时兜底。
  - [ ] 置信合并、永不硬门禁：语义+位置一致→高置信；只有一个→取用；都无→空值。
  - [ ] 模板模式下跳过 RapidLayout/RapidTable（约 10s/13s），只留单次全图 RapidOCR。
  - [ ] 异常路径才裁剪期望区域高 DPI 重 OCR（默认不触发）。
  - [ ] 单测：mock 盒图 + 锚点 → 语义失败时位置命中、漂移时回退语义。

## 遗留待办 / 验证（未定）

- [ ] **真机验收（管理员）**：用多种单据（报关单 / 银行回单 / 收据 / 快递单等）
      验证 rapid 通用性；发现识别明显不对的单据，把结果发回按「数据驱动」方向打磨。
- [ ] **表单标签行 vs 真表格的判别**（顶部 t1 无用表）：当前靠列数一致性切分，
      属已知边界；更难的判别交给 AI 解读/模板步骤。
